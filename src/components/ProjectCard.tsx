...
import { GhostButton, PrimaryButton } from "./Buttons"; // importing GhostButton component from the Buttons file to use as the "View Details" button in the project card
...
function ProjectCard({gen, setGenerations, forCommunity = false} : {gen: Project, setGenerations: React.Dispatch<React.SetStateAction<Project[]>>, forCommunity?:boolean}){
    ...
    const handleDelete = async (id: string) =>{
        const confirm = window.confirm('Are you sure you want tot delete this project?');
        if(!confirm) return;
        console.log(id)
                        ...
                        <ul className={`text-xs ${menuOpen ? 'block' : 'hidden'} verflow-hidden right-0 peer-focus:block hover:block w-40 bg-black/50 backdrop-blur text-white border border-gray-500/50 rounded-lg shadow-md mt-2 py-1 z-10`}>
                            ...
                            {gen.generatedImage && <a href="#" download className="flex gap-2 items-center px-4 py-2 hover:bg-black/10 cursor-pointer"><ImageIcon size={14}/>Download Image</a> }
                              ...
                              {gen.generatedVideo && <a href="#" download className="flex gap-2 items-center px-4 py-2 hover:bg-black/10 cursor-pointer"><PlaySquareIcon size={14}/>Download Video</a> }
                               ...
                               <button onClick={()=> handleDelete(gen.id)} className="w-full flex gap-2 items-cenetr px-4 py-2 hover:bg-red-950/10 text-red-400 cursor-pointer">
