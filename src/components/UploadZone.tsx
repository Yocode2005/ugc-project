...
function UploadZone({label, file, onClear, onChange}:UploadZoneProps) {
  return (
    ...
    {file ? (
        <>
        <img src={URL.createObjectURL(file)} alt="preview" className="absolute inset-0 w-full h-full objet-cover rounded-xl opacity-60" />
        ...
        <div className="absolute bottom-4 left-4 right-4 bg-black/50 backdrop-blur-md p-3 rounded-lg border border-white/10">
            <p className="text-sm fomnt-medium truncate">{file.name}</p>
        </div>
        </>
    ) : (
        <>
        ...
        <h3 className="text-lg font-semibold md-2">{label}</h3>
